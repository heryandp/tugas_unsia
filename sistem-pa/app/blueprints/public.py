...
def landing_page():
    ...
    berita = conn.execute("SELECT * FROM berita ORDER BY RANDOM() LIMIT 3").fetchall()
    galeri = conn.execute("SELECT * FROM galeri ORDER BY id DESC LIMIT 5").fetchall()
...
def profil_pegawai():
    ...
    pegawai = conn.execute(
        "SELECT nama_lengkap, role, nip, pas_foto FROM users ORDER BY role"
    ).fetchall()
    res = conn.execute("SELECT nilai FROM pengaturan WHERE kunci='logo_web'").fetchone()
    logo = res["nilai"] if res else ""
    return render_template("profil_publik.html", pegawai=pegawai, logo=logo)
...
def jadwal_sidang():
    conn = get_db()
    jadwal = conn.execute(
        "SELECT * FROM perkara WHERE tanggal_sidang IS NOT NULL AND tanggal_sidang != '' ORDER BY tanggal_sidang"
    ).fetchall()
    res = conn.execute("SELECT nilai FROM pengaturan WHERE kunci='logo_web'").fetchone()
    logo = res["nilai"] if res else ""
    return render_template("jadwal_publik.html", jadwal=jadwal, logo=logo)
...
def kirim_laporan_tamu():
    ...
    conn = get_db()
    conn.execute(
        "INSERT INTO laporan_tamu (nama_tamu, no_hp, isi_pesan, tanggal, status) VALUES (?, ?, ?, ?, ?)",
        (nama, hp, pesan, tgl, "Menunggu Balasan"),
    )
    conn.commit()
